...
class GeneralController extends Controller
{
    ...
    public function registrationNode(Request $request)
    {
        ...
        if ($category == 'edge') {
            ...
            $data = EdgeComputing::find($request->id);
        } else if ($category == 'node') {
            $path_image = 'iot_node/';
            $data = IOTNode::find($request->id);
        }
    ...
    {
        $name = strtoupper(Str::random(5)) . '-' . rand() . '-' . time() . '.png';
        ...
        Image::make($image->getRealPath())->resize($w, $h, function ($constraint) {
            $constraint->aspectRatio();
        })->save($path . $name);
    ...
    public function newDashboard(Request $request, $id): JsonResponse
{
    ...
    return response()->json([
        'latest' => (count($firstEntryForEachInterval) > 0) ? $firstEntryForEachInterval[0] : null,
